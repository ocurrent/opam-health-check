...
RUN sudo apt update
RUN sudo ln -f /usr/bin/opam-2.3 /usr/bin/opam && opam init --reinit -n
...
FROM debian:12
RUN apt update && apt install netbase dumb-init libev-dev ca-certificates xz-utils tar pixz ugrep -y --no-install-recommends
