...
let get_keyfile workdir username =
  ...
  Server_lib.Workdirs.keyfile ~username workdir
...
let get_log workdir =
  let ilogdir = Server_lib.Workdirs.ilogdir workdir in
...
let admin_action ~on_finished ~conf ~run_trigger workdir body =
  let* resp =
    ...
    | ["set-auto-run-interval"; i] ->
        let* () = Server_lib.Configfile.set_auto_run_interval conf (int_of_string i) in
    ...
    | ["set-processes"; i] ->
        ...
        if i < 0 then
          ...
          let+ () = Server_lib.Configfile.set_processes conf i in
          (fun () -> Lwt.return_none)
    | ["add-ocaml-switch";name;switch] ->
        let switch = Switch.create ~name ~switch in
        let switches = Option.get_or ~default:[] (Server_lib.Configfile.ocaml_switches conf) in
        if List.mem ~eq:Switch.equal switch switches then
          ...
          let+ () = Server_lib.Configfile.set_ocaml_switches conf switches in
          (fun () -> Lwt.return_none)
    ...
    | "set-slack-webhooks"::webhooks ->
        ...
        let+ () = Server_lib.Configfile.set_slack_webhooks conf webhooks in
        (fun () -> Lwt.return_none)
    | ["set-list-command";cmd] ->
        let+ () = Server_lib.Configfile.set_list_command conf cmd in
        (fun () -> Lwt.return_none)
    | ["run"] ->
