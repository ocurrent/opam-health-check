...
let pkg_update ~pool pkg_tbl logdir comp state pkg =
  ...
  let instances =
    ...
    | Some instances -> Instance.create comp state content :: instances
    | None -> [Instance.create comp state content]
  in
...
let fill_pkgs_from_dir ~pool pkg_tbl logdir comp =
  ...
  Prometheus.Gauge.set (Prometheus.Gauge.labels Metrics.statistics [Compiler.to_string comp; "partial"]) (float_of_int (List.length partial_files));
  List.iter (pkg_update ~pool pkg_tbl logdir comp State.Bad) bad_files;
  Prometheus.Gauge.set (Prometheus.Gauge.labels Metrics.statistics [Compiler.to_string comp; "bad"]) (float_of_int (List.length bad_files));
  List.iter (pkg_update ~pool pkg_tbl logdir comp State.NotAvailable) notavailable_files;
  Prometheus.Gauge.set (Prometheus.Gauge.labels Metrics.statistics [Compiler.to_string comp; "not_available"]) (float_of_int (List.length notavailable_files));
  List.iter (pkg_update ~pool pkg_tbl logdir comp State.InternalFailure) internalfailure_files;
  Prometheus.Gauge.set (Prometheus.Gauge.labels Metrics.statistics [Compiler.to_string comp; "internal_failure"]) (float_of_int (List.length internalfailure_files));
  ()

let add_pkg full_name instances acc =
  let pkg = Pkg.name (Pkg.create ~full_name ~instances:[] ~opam:OpamFile.OPAM.empty ~revdeps:0) in (* TODO: Remove this horror *)
  let* acc in
  let* opam = Oca_server.Cache.get_opam cache pkg in
  let+ revdeps = Oca_server.Cache.get_revdeps cache full_name in
  Pkg.create ~full_name ~instances ~opam ~revdeps :: acc
...
let get_log _ ~logdir ~comp ~state ~pkg =
  ...
  match List.find_opt (fun p -> String.equal pkg (Pkg.full_name p)) pkgs with
  | None -> Lwt.return_none
  | Some pkg ->
      let is_instance inst =
        Compiler.equal comp (Instance.compiler inst) &&
        State.equal state (Instance.state inst)
      in
      match List.find_opt is_instance (Pkg.instances pkg) with
      | None -> Lwt.return_none
      | Some instance ->
          let+ content = Instance.content instance in
          Some content
