...
let cache ~conf =
  let os = Server_lib.Configfile.platform_os conf in
  ...
  in
  let dune_cache =
    if Server_lib.Configfile.enable_dune_cache conf
...
let ocluster_build ~cap ~conf ~base_obuilder ~stdout ~stderr c =
  ...
  let pool = Server_lib.Configfile.platform_pool conf in
  ...
  | Ok () ->
      ...
      in
      let timeout =
        let hours = Server_lib.Configfile.job_timeout conf in
...
let failure_kind conf ~pkg logfile =
  ...
  in
  let timeout = Server_lib.Configfile.job_timeout conf in
...
let with_test ~conf pkg =
  if Server_lib.Configfile.with_test conf then
    with_test pkg
  else
    ""
...
let with_lower_bound ~conf pkg =
  if Server_lib.Configfile.with_lower_bound conf then
...
res=$?
if [ $res = 31 ]; then
    if opam show -f x-ci-accept-failures: "|}^pkg^{|" | grep -q '"|}^Server_lib.Configfile.platform_distribution conf^{|"'; then
...
let run_job ~cap ~conf ~pool ~stderr ~base_obuilder ~switch ~num logdir pkg =
  Lwt_pool.use pool begin fun () ->
    ...
    let logfile = Server_lib.Workdirs.tmplogfile ~pkg ~switch logdir in
    let* v =
    ...
    | Ok () ->
        ...
        Lwt_unix.rename (Fpath.to_string logfile) (Fpath.to_string (Server_lib.Workdirs.tmpgoodlog ~pkg ~switch logdir))
    | Error () ->
        ...
        | `Partial ->
            ...
            Lwt_unix.rename (Fpath.to_string logfile) (Fpath.to_string (Server_lib.Workdirs.tmppartiallog ~pkg ~switch logdir))
        | `Failure ->
            let* () = Lwt_io.write_line stderr ("["^num^"] failed.") in
            Lwt_unix.rename (Fpath.to_string logfile) (Fpath.to_string (Server_lib.Workdirs.tmpbadlog ~pkg ~switch logdir))
        | `NotAvailable ->
            let* () = Lwt_io.write_line stderr ("["^num^"] finished with not available.") in
            Lwt_unix.rename (Fpath.to_string logfile) (Fpath.to_string (Server_lib.Workdirs.tmpnotavailablelog ~pkg ~switch logdir))
        | `Other | `AcceptFailures | `Timeout ->
            let* () = Lwt_io.write_line stderr ("["^num^"] finished with an internal failure.") in
            Lwt_unix.rename (Fpath.to_string logfile) (Fpath.to_string (Server_lib.Workdirs.tmpinternalfailurelog ~pkg ~switch logdir))
        end
  end
...
let get_obuilder ~conf ~opam_repo ~opam_repo_commit ~extra_repos switch =
  ...
  let os = Server_lib.Configfile.platform_os conf in
  let distribution = Server_lib.Configfile.platform_distribution conf in
  ...
  stage ~from begin
    ...
    (match Server_lib.Configfile.extra_command conf with
     | Some c -> [run ~cache ~network "%s" c]
     | None -> []
    ) @
    (if Server_lib.Configfile.enable_dune_cache conf then
...
let get_pkgs ~debug ~cap ~conf ~stderr (switch, base_obuilder) =
  ...
  let* pkgs = ocluster_build_str ~important:true ~debug ~cap ~conf ~base_obuilder ~stderr ~default:None (Server_lib.Configfile.list_command conf) in
...
let with_stderr ~start_time workdir f =
  let* () = Oca_lib.mkdir_p (Server_lib.Workdirs.ilogdir workdir) in
  let logfile = Server_lib.Workdirs.new_ilogfile ~start_time workdir in
...
let get_metadata ~debug ~jobs ~cap ~conf ~pool ~stderr logdir (_, base_obuilder) pkgs =
  let get_revdeps ~base_obuilder ~pkgname ~pkg ~logdir =
    ...
    Lwt_io.with_file ~mode:Lwt_io.output (Fpath.to_string (Server_lib.Workdirs.tmprevdepsfile ~pkg logdir)) (fun c ->
  ...
  let get_latest_metadata ~base_obuilder ~pkgname ~logdir = (* TODO: Get this locally by merging all the repository and parsing the opam files using opam-core *)
    ...
    in
    Lwt_io.with_file ~mode:Lwt_io.output (Fpath.to_string (Server_lib.Workdirs.tmpopamfile ~pkg:pkgname logdir)) (fun c ->
...
let get_commit_hash_default conf =
  let github = Server_lib.Configfile.default_repository conf in
...
let get_commit_hash_extra_repos conf =
  Lwt_list.map_s begin fun repository ->
    ...
    let+ hash = get_commit_hash github in
    repository, hash
  end (Server_lib.Configfile.extra_repositories conf)

let move_tmpdirs_to_final ~switches logdir workdir =
  let metadatadir = Server_lib.Workdirs.metadatadir workdir in
  let tmpmetadatadir = Server_lib.Workdirs.tmpmetadatadir logdir in
  let tmpdir = Server_lib.Workdirs.tmpdir logdir in
  let switches = List.map Switch.name switches in
  let* () = Server_lib.Workdirs.logdir_move ~switches logdir in
...
let trigger_slack_webhooks ~stderr ~old_logdir ~new_logdir conf =
  let public_url = Server_lib.Configfile.public_url conf in
  ...
  in
  Server_lib.Configfile.slack_webhooks conf |>
...
let update_docker_image conf =
  ...
  let get_latest_image ~image =
    ...
    let os = Server_lib.Configfile.platform_os conf in
    let arch = match Server_lib.Configfile.platform_arch conf with
      | "x86_64" -> "amd64"
      | arch -> arch
    in
    ...
    | Error e ->
       ...
       Lwt.return_none
  in
  let image = Server_lib.Configfile.platform_image conf in
  match String.split_on_char '@' image with
  | [] -> assert false
  | [image] ->
      let* v =get_latest_image ~image in
      begin match v with
      | Some image -> Server_lib.Configfile.set_platform_image conf image
...
let run ~debug ~cap_file ~on_finished ~conf cache workdir =
  let switches = Option.get_exn_or "no switches" (Server_lib.Configfile.ocaml_switches conf) in
  if !run_locked then
    ...
    with_stderr ~start_time workdir begin fun ~stderr ->
      Lwt.catch (fun () ->
        ...
        | switch::_ ->
            ...
            let compressed = Server_lib.Configfile.enable_logs_compression conf in
            let old_logdir = List.head_opt old_logdir in
            let new_logdir = Server_lib.Workdirs.new_logdir ~compressed ~hash:opam_repo_commit ~start_time workdir in
            let* () = Server_lib.Workdirs.init_base_jobs ~switches:switches' new_logdir in
            let pool = Lwt_pool.create (Server_lib.Configfile.processes conf) (fun () -> Lwt.return_unit) in
