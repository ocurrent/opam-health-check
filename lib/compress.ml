(* File copied from bin/easy.ml from the decompress library *)
(* Copyright (c) 2014 oklm-wsh *)

module Stdlib_buffer = Buffer
open Decompress

type t = Compressed of string
...
let compress ?(level = 4) data =
  ...
  |> function
  | Ok _ -> Compressed (Stdlib_buffer.contents res) | Error exn -> raise (Deflate_error exn)

let uncompress (Compressed data) =
  let input_buffer = Bytes.create 0xFFFF in
  ...
  |> function
  | Ok _ -> Stdlib_buffer.contents res | Error exn -> raise (Inflate_error exn)
